...
- **NTP 精确定时** — 基于网络时间的定时消息调度器（修复 sleep 钳制导致的 ~1kHz 忙等死循环，踩到整分钟边界时任务仍能准点触发）
...
> 最简单的部署方式，qqbot-fire + NapCat 一键启动。

**环境要求：** Docker + Docker Compose

```bash
# 1. 克隆项目
git clone https://github.com/你的用户名/qqbot-fire.git
cd qqbot-fire
...
# 重新构建 qqbot-fire（代码更新后）
docker compose up -d --build qqbot-fire

# 单独查看某个容器日志
docker compose logs -f qqbot-fire
docker compose logs -f napcat

# 进入 qqbot-fire 容器交互式控制台
docker attach qqbot-fire
...
- **仪表盘** — Bot 状态卡片一览，快速连接/断开，5 秒自动刷新；NapCat 运行实例面板展示 WS/HTTP/WebUI 地址与 WebUI Token，一键复制
...
└── vite.config.ts                     # Vite 配置 (/api 代理到 :9988)
logs/                                      # 运行时自动创建
├── qqbot-fire.log                     # 主日志
...
└── gson-2.12.1.jar
Dockerfile                                     # 多阶段构建 (JDK build -> JRE runtime)
docker-compose.yml                             # NapCat + qqbot-fire 编排
...
- [x] **Docker 部署** — Dockerfile + docker-compose 与 NapCat 一键编排（已完成）
