...
export default function QQHistoryInput({ storageKey, placeholder = 'QQ 号', value, onChange, className = '' }: Props) {
  const [history, setHistory] = useLocalStorage<string[]>('qqHistory_' + storageKey, [])
  ...
  return (
    <div ref={containerRef} className={`relative ${className}`}>
      ...
      {open && (
        ...
        >
          {history.length === 0 ? (
            <div className="px-3 py-2 text-center text-text-muted text-xs">暂无历史记录</div>
          ) : (
            history.map(v => (
              <div key={v} className="flex justify-between items-center px-3 py-2 hover:bg-white/10 cursor-pointer text-sm text-text-primary transition-colors">
                <span onClick={() => { onChange(v); setOpen(false); }}>{v}</span>
                <span
                  onClick={(e) => { e.stopPropagation(); removeFromHistory(v); }}
