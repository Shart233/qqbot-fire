...
export default function ConsolePage() {
  const [output, setOutput] = useState('QQBot-Fire Web 控制台\n输入 /help 查看命令列表\n')
  ...
  const handleExec = async () => {
    ...
    if (data && data.output) {
      setOutput(prev => prev + data.output + (data.output.endsWith('\n') ? '' : '\n'))
    }
    setTimeout(scrollToBottom, 0)
  ...
  return (
    <div className="flex flex-col" style={{ height: 'calc(100vh - 130px)' }}>
      ...
      >
        <span className="font-mono text-[0.82rem] text-accent mr-2 select-none whitespace-nowrap">{prompt}</span>
        <input
          ref={inputRef}
          type="text"
          value={input}
