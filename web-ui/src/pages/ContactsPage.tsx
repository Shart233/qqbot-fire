...
import { listBots, getFriends, getGroups, getGroupMembers } from '../api/endpoints'
...
export default function ContactsPage() {
  ...
  // Group members
  const [memberGroup, setMemberGroup] = useState<{ groupId: number; groupName: string } | null>(null)
  ...
  return (
    <div>
      <PageHeader title="好友与群">
        <BotSelect storageKey="select_contactBotSelect" connectedOnly value={bot} onChange={setBot} />
      </PageHeader>

      <Tabs selectedKey={tab} onSelectionChange={(key) => setTab(key as string)}>
        <Tabs.List>
          <Tabs.Tab id="friends">好友列表<Tabs.Indicator /></Tabs.Tab>
          <Tabs.Tab id="groups">群列表<Tabs.Indicator /></Tabs.Tab>
        </Tabs.List>
                  ...
                  <Table.Body>
                    {groups.map(g => (
                      <Table.Row key={g.groupId} id={g.groupId}>
                        ...
                        <Table.Cell>{g.memberCount}/{g.maxMemberCount}</Table.Cell>
                        <Table.Cell>
                          <Button size="sm" variant="ghost" onPress={() => handleLoadMembers(g.groupId, g.groupName)}>查看成员</Button>
                        </Table.Cell>
                      </Table.Row>
                    ))}
                  </Table.Body>
              ...
              {memberGroup && (
                ...
                >
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <Button size="sm" variant="ghost" onPress={() => setMemberGroup(null)}>关闭</Button>
                  </div>
                  {membersLoading ? (
                            ...
                            <Table.Row key={m.userId} id={m.userId}>
                              ...
                              <Table.Cell>
                                <Chip
                                  size="sm"
                                  variant="soft"
                                  color={m.role === 'owner' ? 'danger' : m.role === 'admin' ? 'warning' : 'default'}
                                >
