...
import { listBots, connectBot, disconnectBot, connectAll, disconnectAll } from '../api/endpoints'
  ...
  return (
    <div>
      <PageHeader title="仪表盘">
        <Button size="sm" variant="ghost" className="text-success" onPress={handleConnectAll}>全部连接</Button>
        <Button size="sm" variant="danger-soft" onPress={handleDisconnectAll}>全部断开</Button>
      </PageHeader>
          ...
          {cachedBots.map((bot) => (
            <GlassCard key={bot.name} className="p-4 flex flex-col gap-3">
              <div className="flex items-center justify-between">
                <span className="text-text-primary font-semibold text-sm truncate">{bot.name}</span>
                <Chip
                  size="sm"
                  variant="soft"
              ...
              <div className="text-sm text-text-secondary space-y-1">
                <div className="flex items-center gap-2">
                  <span className={`w-2 h-2 rounded-full inline-block ${bot.connected ? 'bg-success animate-[pulse-dot_2s_ease_infinite]' : 'bg-text-muted'}`} />
